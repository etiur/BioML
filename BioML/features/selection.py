...
def arg_parse():
    ...
    parser.add_argument("-s", "--scaler", required=False, default="zscore", choices=("robust", "zscore", "minmax"),
                        help="Choose one of the scaler available in scikit-learn, defaults to StandardScaler")
    ...
    parser.add_argument("-st", "--split_strategy", required=False, default="stratifiedkfold", 
                        choices=("cluster", "stratifiedkfold", "kfold"), help="The strategy to split the data")
    ...
    parser.add_argument("-ti", "--stratified", action="store_true", required=False, help="Use stratified for the " \
                                                                                        "cluster spliting strategy, default is False")
    ...
    return [args.features, args.label, args.variance_threshold, args.feature_range, args.num_thread, args.scaler,
            ...
            args.seed, args.problem, args.outliers, args.sheet_name, args.split_strategy, args.cluster, args.shuffle,
            args.stratified, args.kfold_parameters]
    ...
    def read_feature(self, features: str | pd.DataFrame | list | np.ndarray) -> pd.DataFrame:
        ...
        match features:
            case str() | Path() as feat if str(feat).endswith(".csv"):
                frame = pd.read_csv(f"{features}", index_col=0)
                frame.columns = frame.columns.astype(str)
                return frame # the first column should contain the sample names
            case str() | Path() as feat if str(feat).endswith(".xlsx"):
                sheet = self.sheet if self.sheet else 0
                frame = pd.read_excel(f"{features}", index_col=0, sheet_name=sheet)
                frame.columns = frame.columns.astype(str)
                return frame # the first column should contain the sample names
...
def main():
    ...
    plot_num_features, seed, problem, outliers, sheets, split_strategy, cluster, shuffle, stratified, \
    kfold = arg_parse()
    ...
    if split_strategy == "cluster":
        if not Path(cluster).exists():
            raise FileNotFoundError(f"cluster file {cluster} does not exist and the strategy was cluster")
        clust = split.ClusterSpliter(cluster, num_split, shuffle=shuffle, random_state=seed, stratified=stratified)
        X_train, X_test, y_train, y_test = clust.train_test_split(training_data.features, training_data.label, test_size=test_size)
    
    elif split_strategy == "stratifiedkfold" and "problem"  == "classification":
        X_train, X_test, y_train, y_test = train_test_split(training_data.features, training_data.label, test_size=test_size, 
                                                            random_state=seed, shuffle=shuffle, stratify=training_data.label)
    else:            
        X_train, X_test, y_train, y_test = train_test_split(training_data.features, training_data.label, test_size=test_size,
    ...
    selection.construct_features(training_data.features, X_train, X_test, y_train, feature_range, 
