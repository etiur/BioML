...
class MmseqsClustering:
    ...
    @classmethod
    def easy_generate_pssm(cls, input_file: str | Path, database_input: str | Path, 
        ...
        **search_kwags : dict, optional
        ...
        query_db = Path(input_file).with_suffix("")/"querydb"
        input_database = Path(database_input)
        search_db = input_database.with_suffix("")/"searchdb"
        # generate the databases using the fasta files from input and the search database like uniref
        ...
        if generate_searchdb:
            ...
            if not search_db.exists():
                raise FileNotFoundError("The search database does not exist. Set generate_searchdb to True to create it.")
